...
export const Generators = {
    ...
    SpaceStation: (): VoxelData[] => {
        ...
        // Central Sphere (Core)
        generateSphere(map, CX, CY, CZ, 4, COLORS.WHITE);
        generateSphere(map, CX, CY, CZ, 3, COLORS.CYAN); // Glowing core inside
        
        // Solar Panels (X axis)
        for (let x = 4; x <= 14; x++) {
            for (let z = -4; z <= 4; z++) {
                setBlock(map, CX + x, CY, CZ + z, COLORS.CYAN);
                setBlock(map, CX - x, CY, CZ + z, COLORS.CYAN);
                // Panel borders
                if (z === -4 || z === 4 || x === 14) {
                   setBlock(map, CX + x, CY, CZ + z, COLORS.BLACK);
                   setBlock(map, CX - x, CY, CZ + z, COLORS.BLACK);
                }
            }
        }
        
        // Vertical Pillars (Y axis)
        for (let y = -6; y <= 6; y++) {
            setBlock(map, CX, CY + y, CZ, COLORS.WHITE);
            if (y === 6 || y === -6) {
                generateSphere(map, CX, CY + y, CZ, 1.5, COLORS.GOLD); // Comms/Sensor arrays
            }
        }
        ...
        for (let i = 0; i < 360; i += 30) {
            ...
            const rx = Math.cos(rad) * 6;
            const rz = Math.sin(rad) * 6;
            setBlock(map, CX + rx, CY + 2, CZ + rz, COLORS.LIGHT);
            setBlock(map, CX + rx, CY - 2, CZ + rz, COLORS.LIGHT);
        }
